  ...
  return (
    <div className={`min-h-screen bg-slate-950 selection:bg-indigo-500/30 overflow-x-hidden animated-bg relative ${templateClass}`}>
      ...
      <motion.button
        ...
        className="fixed top-1/2 -left-2 z-[var(--z-nav)] -rotate-90 origin-left hidden sm:flex items-center gap-3 bg-indigo-600 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-t-2xl sm:rounded-t-3xl shadow-[0_-10px_40px_rgba(79,70,229,0.3)] border-x border-t border-white/20 hover:px-8 sm:hover:px-10 transition-all group"
      >
        <Sparkles size={16} className="animate-pulse text-indigo-200" />
        <span className="text-[10px] sm:text-xs font-black uppercase tracking-[0.15em] whitespace-nowrap">استشارة ذكية مجانية</span>
      </motion.button>

      <nav className="fixed top-3 sm:top-6 left-0 right-0 z-[var(--z-nav)] px-3 sm:px-6">
        <div className="container mx-auto max-w-7xl">
          <div className="flex justify-between items-center px-4 sm:px-6 md:px-8 py-3 sm:py-4 glass-panel border border-white/10 rounded-2xl sm:rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden group/nav">
            ...
            <div onClick={(e) => scrollToSection(e, '#hero')} className="flex items-center gap-4 group cursor-pointer relative z-10">
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-indigo-600 rounded-xl sm:rounded-2xl flex items-center justify-center text-white shadow-[0_0_20px_rgba(79,70,229,0.4)] group-hover:rotate-[15deg] transition-all duration-500">
                <Zap className="text-white fill-current" size={20} />
              </div>
              <div className="flex flex-col items-start leading-none text-right">
                <span className="text-lg sm:text-xl font-black text-white tracking-tighter uppercase leading-none">REFF<span className="text-indigo-500">AA</span></span>
                <span className="hidden sm:inline text-[9px] font-black text-slate-500 uppercase tracking-[0.3em] mt-1">{profile?.nameEn?.split(' ')[0] || 'NR'} STRATEGY</span>
              </div>
            </div>
            ...
            <button
              ...
              className={`lg:hidden p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border transition-all duration-500 relative z-10 ${isMenuOpen ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg shadow-indigo-600/40' : 'bg-white/5 border-white/10 text-white'}`}
            >
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              ...
              className="fixed inset-0 z-[350] lg:hidden bg-slate-950 flex flex-col pt-24 sm:pt-28 md:pt-32 pb-8 sm:pb-12 px-6 sm:px-8 overflow-y-auto"
            >
              ...
              <div className="flex flex-col gap-3 sm:gap-4 text-right" dir="rtl">
                {navLinks.map((link, i) => (
                  <motion.button
                    key={i}
                    ...
                    onClick={(e) => scrollToSection(e, link.href)}
                    className="group flex items-center justify-between py-5 sm:py-6 border-b border-white/5"
                  >
                    <span className="text-3xl sm:text-4xl md:text-5xl font-black text-white group-hover:text-indigo-500 transition-all uppercase tracking-tighter">
                      {link.name}
                    </span>
                    <ArrowRight className="text-slate-800 group-hover:text-indigo-500 group-hover:translate-x-[-10px] transition-all rotate-180" size={28} />
                  </motion.button>
                ))}
              </div>
              ...
              >
                ...
                >
                  <Bot size={24} className="sm:w-[28px] sm:h-[28px]" />
                  استشر المساعد الذكي
                </button>

                <div className="flex justify-center gap-6 sm:gap-8 md:gap-10">
                  <a href={facebookUrl} target="_blank" rel="noreferrer" className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-white/5 flex items-center justify-center text-slate-400 hover:bg-indigo-600 hover:text-white transition-all"><Facebook size={20} className="sm:w-[24px] sm:h-[24px]" /></a>
                  <a href={linkedinUrl} target="_blank" rel="noreferrer" className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-white/5 flex items-center justify-center text-slate-400 hover:bg-indigo-600 hover:text-white transition-all"><Linkedin size={20} className="sm:w-[24px] sm:h-[24px]" /></a>
                  <a href={emailUrl} className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-white/5 flex items-center justify-center text-slate-400 hover:bg-indigo-600 hover:text-white transition-all"><Mail size={20} className="sm:w-[24px] sm:h-[24px]" /></a>
                </div>
              </motion.div>
            </motion.div>
          )}
