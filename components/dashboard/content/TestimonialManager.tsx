...
const TestimonialManager: React.FC = () => {
    ...
    const testimonials = (siteData as any).testimonials || [];
    ...
    const handleDelete = (index: number) => {
        if (window.confirm('هل أنت متأكد من حذف هذا التقييم؟')) {
            const newItems = testimonials.filter((_: any, i: number) => i !== index);
            updateSiteData({ testimonials: newItems } as any);
        }
    };
    ...
    const addNew = () => {
        if (newItem.name && newItem.text) {
            updateSiteData({ testimonials: [...testimonials, newItem] } as any);
                    ...
                    <div key={index} className="bg-slate-900 border border-white/5 p-4 rounded-xl group hover:border-white/10 transition-colors">
                        {editingIndex === index ? (
                            <div className="space-y-3">
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-950 border border-indigo-500/50 rounded-lg p-2 text-white"
                                    />
                                </div>
                                <textarea
                                    value={editForm.text}
                            ...
                            <div>
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-indigo-400 font-bold">{item.role}</p>
                                        </div>
                                    </div>
                                ...
                                <p className="text-xs text-slate-400 italic leading-relaxed">"{item.text}"</p>
                            </div>
                        )}
                    </div>
                ))}
